    ...
    list.innerHTML = data.item.length === 0
      ...
      : data.item.map(n => `
        <a href="${n.pesanan_id ? 'pesanan_detail.php?id=' + n.pesanan_id : '#'}"
           class="notif-item ${n.dibaca == 0 ? 'baru' : ''}" data-id="${n.id}">
...
// Klik satu notifikasi -> tandai HANYA notifikasi itu sebagai dibaca
document.getElementById('notifList')?.addEventListener('click', (ev) => {
  const item = ev.target.closest('.notif-item');
  if (!item || !item.dataset.id) return;
  const body = 'aksi=baca&id=' + item.dataset.id;
  const opsi = { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body };
  item.classList.remove('baru');
  if (item.getAttribute('href') === '#') {
    ev.preventDefault();
    fetch('api/notifikasi.php', opsi).then(muatNotif);
  } else {
    // biarkan pindah halaman; keepalive memastikan request tetap terkirim
    fetch('api/notifikasi.php', { ...opsi, keepalive: true });
  }
});
muatNotif();
