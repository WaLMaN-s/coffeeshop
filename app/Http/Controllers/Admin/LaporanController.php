...
class LaporanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $qs   = http_build_query(['periode' => $data['periode'], 'mulai' => $data['mulai'], 'sampai' => $data['sampai']]);
    ...
    {
        $data     = $this->hitungLaporan($request);
        $namaFile = 'laporan_' . $data['mulai'] . '_sd_' . $data['sampai'] . '.xls';

        $pengaturan = get_pengaturan(db());
        $namaToko   = $pengaturan['nama_toko'] ?? 'Lorong Kopi';

        $isi = "\xEF\xBB\xBF" . view('admin.laporan_excel', $data + ['namaToko' => $namaToko])->render();
     ...
     * (port dari admin/includes/laporan_data.php).
     */
    private function hitungLaporan(Request $request): array
    {
        ...
        return compact('periode', 'mulai', 'sampai', 'ringkasan', 'perHari', 'terlarisLaporan');
    }
}
