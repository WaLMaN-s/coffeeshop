...
class PesananController extends Controller
{
    public function daftar(Request $request)
    {
        ...
        return view('kasir.pesanan', [
            'pageTitle' => 'Pesanan',
            'active'    => 'pesanan',
            'q' => $q, 'tanggal' => $tanggal, 'status' => $status,
            'daftar' => $daftar,
        ]);
    }
    ...
    public function detail(Request $request)
    {
        ...
        if ($request->isMethod('post') && $request->input('aksi') === 'ubah_status') {
            ...
            return redirect('kasir/pesanan_detail.php?id=' . $id);
        }

        /* ---------- Verifikasi pembayaran cepat ---------- */
        if ($request->isMethod('post') && $request->input('aksi') === 'verifikasi_bayar') {
            $bayarId = (int) $request->input('bayar_id');
        ...
        /* ---------- Data ---------- */
        $data = $this->ambilPesanan($db, $id);
        if (!$data) {
            set_flash('gagal', 'Pesanan tidak ditemukan.');
            return redirect('kasir/pesanan.php');
        }
        [$pesanan, $item, $bayar] = $data;
        ...
        return view('kasir.pesanan_detail', [
    ...
    public function struk(Request $request)
    {
        ...
        return view('kasir.struk', compact('pesanan', 'item', 'bayar', 'pengaturan', 'namaToko'));
    }

    /** @return array{0: array, 1: array, 2: array|false}|null */
    private function ambilPesanan(\PDO $db, int $id): ?array
    {
        $stmt = $db->prepare("
