...
/* Opsi minuman — sama dengan sisi pelanggan (includes/site_init.php). */
const POS_UKURAN   = ['Regular' => 0, 'Large' => 5000];
const POS_SAJI     = ['Dingin', 'Panas'];
const POS_GULA     = ['Normal Sugar', 'Less Sugar', 'No Sugar'];
const POS_MINUMAN  = ['Coffee', 'Non Coffee', 'Tea'];

/* ---------- Simpan pesanan dari kasir ---------- */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $namaTamu = trim($_POST['nama'] ?? '');
    $mejaId   = (int) ($_POST['meja_id'] ?? 0);
    $metode   = ($_POST['metode'] ?? 'cash') === 'qris' ? 'qris' : 'cash';
    $lunas    = !empty($_POST['lunas']);
    ...
    if (!is_array($itemJson) || !$itemJson) {
        ...
        header('Location: pesanan_baru.php');
        exit;
    }
    $db->beginTransaction();
    try {
        ...
        foreach ($itemJson as $it) {
            ...
            $harga   = (float) $m['harga'];
            if ($minuman) {
                $ukuran = array_key_exists($it['ukuran'] ?? '', POS_UKURAN) ? $it['ukuran'] : 'Regular';
                $saji   = in_array($it['saji'] ?? '', POS_SAJI, true) ? $it['saji'] : 'Dingin';
                $harga += POS_UKURAN[$ukuran];
                if ($m['tanpa_gula']) {
                    // Espresso, Americano, dll — tidak pakai opsi gula
                    $opsi = $ukuran . ' · ' . $saji;
                } else {
                    $gula = in_array($it['gula'] ?? '', POS_GULA, true) ? $it['gula'] : 'Normal Sugar';
        ...
        header('Location: pesanan_detail.php?id=' . $pesananId);
        exit;
    } catch (Throwable $e) {
        $db->rollBack();
...
let posItems = []; // {menu_id, nama, harga, minuman, jumlah, ukuran, saji, gula}
